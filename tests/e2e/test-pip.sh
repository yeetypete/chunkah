...
RUN dnf install -y python3-pip python3-requests uv && dnf clean all && \
    ...
    uv venv /opt/venv && \
    uv pip install --python /opt/venv --no-cache --compile-bytecode markdown-it-py rich
EOF
...
assert_has_components "${CHUNKED_IMAGE}" "pip/numpy" "pip/rich" "pip/markdown-it-py" "rpm/python-requests"
